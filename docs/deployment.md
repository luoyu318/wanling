...
万灵服务端支持两条部署路线，**根据你的场景选一条**：

| 路线 | 适用场景 | 不适用 |
|---|---|---|
| **A. Docker Compose**（推荐） | 全新机器、快速部署、单机、跨平台（Linux/Windows/Mac） | 已有 PG/Redis 实例、要求精细控制每个组件、k8s 编排 |
| **B. 源码 + systemd** | 已有 PG/Redis、运维体系基于 systemd、不想引入 Docker | Windows 服务器（systemd 不支持） |

**怎么选**：
- 不确定 → 选 A（90% 场景的最优解）
- 公司有 DBA 团队管 PG / 已有 Redis 集群 → 选 B
- Windows Server 部署 → 选 A

**通用环境要求**：

| 组件 | 版本 | 路线 A | 路线 B |
|---|---|---|---|
| Docker Engine | >= 24 | ✅ 必需 | ❌ 不需要 |
| Docker Compose v2 | >= 2.20 | ✅ 必需 | ❌ 不需要 |
| Go | >= 1.25 | ❌ 不需要（镜像内编译） | ✅ 必需（本机编译） |
| PostgreSQL | >= 15 | ❌ 不需要（容器起） | ✅ 必需 |
| Redis | >= 7 | ❌ 不需要（容器起） | ✅ 可选（不装也能跑，自动降级） |
| Flutter | >= 3.44 | APP 构建时需要（路线无关） | APP 构建时需要 |
| Python（hermes 插件） | >= 3.11 | hermes 端需要（路线无关） | hermes 端需要 |

---

## 2. 路线 A：Docker Compose 部署（推荐）
...
docker compose up -d            # migrate one-shot 会自动跑新 migrations
```
...
## 3. 路线 B：源码 + systemd 部署

### 3.1 前置条件

- Linux 服务器（systemd 支持）
- Go >= 1.25
- PostgreSQL >= 15（已安装或自装）
- Redis >= 7（可选，不装会自动降级）
...
```bash
curl http://localhost:18008/health    # 应返回 {"status":"ok"}
```

### 3.9 升级

`scripts/deploy.sh` 已经封装了完整的升级流程（编译 → rsync → migrate → restart → 健康检查）：
...
ssh user@server "cd /usr/local/wanling && ./bin/migrate up --env=etc/.env"

# 3. 原子替换 + 重启
ssh user@server "cd /usr/local/wanling/bin && mv wanling-server wanling-server.prev && mv wanling-server.new wanling-server"
...
## 4. Nginx 反向代理 + TLS（两条路线通用）

生产环境强烈建议前置 nginx 做 TLS 终止 + WebSocket 升级。**两条部署路线都需要**。
...
> **明文 HTTP 允许**：APP 默认连 `http://localhost:18008` 或自定义服务器地址。若服务端未上 TLS，需在 `android/app/src/main/res/xml/network_security_config.xml` 配置 `cleartextTrafficPermitted`，并在 manifest 引用 `android:networkSecurityConfig="@xml/network_security_config"`。生产环境强烈建议服务端上 TLS（见第 4 节 Nginx）。

---

## 6. Hermes Plugin 部署（路线无关）
...
默认使用本地存储（`STORAGE_PATH` 环境变量）。

| 路线 | 默认路径 |
|---|---|
| Docker Compose prod | `/app/uploads`（容器内，映射到 named volume `uploads`） |
| Docker Compose dev | `/app/uploads`（容器内，映射到 `./data/uploads`） |
| 源码 + systemd | `/usr/local/wanling/uploads`（在 `.env` 里配 `STORAGE_PATH`） |
...
```bash
# 路线 A（Docker，uploads 在 named volume）：
...
```cron
# 路线 A（Docker，假设 compose 文件在 /opt/wanling/）：
...
| 指标 | 来源 | 告警阈值 |
|------|------|---------|
| 服务端是否存活 | 路线 A: `docker compose ps` / 路线 B: `systemctl is-active wanling-server` | 非 healthy / 非 active |
| WebSocket 连接数 | Hub `sync.Map` Range 计数 | 突降 50% |
| PG 连接数 | `pg_stat_activity` | > 100 |
| 磁盘（uploads 目录） | `df -h` | > 80% |
...
> **完全静默**，不污染 journal / docker logs。判定用 gin 的 `c.FullPath()`（命中 NoRoute 返回空串）。
> file_handler 的 5 处错误都用 `log.Printf` 带 `[upload]`/`[download]` 前缀打 stderr，
> 可用 `docker compose logs server | grep -E "\[upload\]|\[download\]"`（路线 A）
> 或 `journalctl -u wanling-server | grep -E "\[upload\]|\[download\]"`（路线 B）过滤。
...
- [ ] 防火墙仅放通 443（Nginx），18008 仅监听 127.0.0.1（路线 B）/ 不暴露公网（路线 A 由 VPS 防火墙控）
...
| 症状 | 检查项 |
|------|--------|
| 客户端连不上 | 路线 A: `docker compose ps` / 路线 B: `systemctl status wanling-server`；`ss -tlnp \| grep 18008` |
| 登录失败 | PG 是否可达：路线 A: `docker compose exec postgres psql -U wanling -c "SELECT 1"` / 路线 B: `psql -U agent -d wanling -c "SELECT 1"` |
| WS 连接断开 | 路线 A: `docker compose logs server \| grep -i error` / 路线 B: `journalctl -u wanling-server \| grep -i error` |
...
| migrate 卡住不退出 | 路线 A: `docker compose logs migrate`（正常应 Exited 0）；migrate 失败会阻塞 server 启动 |
