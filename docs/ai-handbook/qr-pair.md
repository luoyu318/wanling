...
3. app 在 `PairSelectAgentPage` 选已有 agent（弹"重置密钥"确认）或新建 → `POST /api/pair/tickets/:id/complete`。新建分支读 `ticket.type` 建 agent,实现 agent 类型标签全链路透传(opencode agent 扫码配对后 APP 可识别)。
4. hermes 端 2s 短轮询 `GET /api/pair/tickets/:id`，completed 时拿凭据(`agent_id` / `secret_key` / `owner_user_id` / `owner_conv_id`),**领完即焚**（secret_key 领走后清空）。`owner_conv_id` 写入 `.env` `WANLING_HOME_CONV`,adapter 启动直接读,无需手动 find_or_create。
