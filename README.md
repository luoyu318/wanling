...
- 💬 **IM 风格对话**：未读红点、置顶、消息多选删除、撤回、长按菜单、Markdown / LaTeX / 代码块高亮 / 图片 / 文件渲染、**图片画廊**（点击图片全屏查看，会话内左右滑动切换，双指缩放 + 放大态跟随翻页、**长按保存到相册**）
- 👥 **多账号切换**：保存多个登录（账号 + 服务器地址 + 自定义标记），登录后从「我的」一键切换账号，无需重输密码；服务器地址随账号自动切换
...
- 📲 **离线推送**：Android 前台服务保活 WS，APP 被杀也能收到通知（点击直达会话）。**富样式通知**：带 Agent 头像 + `[N条]` 未读计数前缀，多条消息折叠展示
