...
class MessageStore:
    ...
    def get_conv_id_by_user(self, user_id: str) -> str:
        """按 user_id 反查 conv_id。一个 user 在同一 agent 下通常只有一个会话。"""
        for cid, meta in self._convs.items():
    ...
    def pop_all_unread_grouped_by_user(self) -> dict[str, list[dict[str, Any]]]:
        """取出所有会话的未读，按 user_id 分组返回。"""
        result: dict[str, list[dict[str, Any]]] = {}
        for cid in list(self._unread.keys()):
            msgs = self._unread.pop(cid, [])
            if not msgs:
                continue
            uid = self._convs.get(cid).user_id if self._convs.get(cid) else cid
            result[uid] = msgs
        return result
