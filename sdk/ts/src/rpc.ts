...
export class RPCDispatcher {
  private handlers = new Map<string, { handler: RPCHandler; timeoutHintMs: number }>()

  register(name: string, handler: RPCHandler, opts?: { timeoutHintMs?: number }): void {
    this.handlers.set(name, { handler, timeoutHintMs: opts?.timeoutHintMs ?? DEFAULT_TIMEOUT_HINT_MS })
  }
  ...
  listMethods(): Array<{ name: string; timeout_hint_ms: number }> {
    return Array.from(this.handlers.entries()).map(([name, def]) => ({
      name,
      timeout_hint_ms: def.timeoutHintMs,
    }))
  }
  ...
  async dispatch(call: JSONRPCRequest): Promise<JSONRPCResponse> {
    ...
    try {
      ...
      if (e instanceof RPCError) {
        return { jsonrpc: "2.0", id: call.id, error: { code: e.code, message: e.message } }
      }
      const message = e instanceof Error ? e.message : String(e)
      return { jsonrpc: "2.0", id: call.id, error: { code: ERR_INTERNAL, message } }
    }
  }
}
