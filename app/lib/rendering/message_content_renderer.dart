...
class MessageRenderContext {
  ...
  /// 当前会话 ID。权限/问题卡片 renderer 提交回复时用它作为发送目标
  /// （走 apiProvider.sendMessage(convId, content)）。由 MessageBubble 从
  /// message.conversationId 注入。
  final String convId;

  /// 当前消息 ID。task 卡片 renderer 跳转「子 Agent 详情页」时用它作为
  ...
  /// 文件卡片点击回调: (fileId, filename, mimeType, fileSize)。
  /// 由 ChatPage 注入下载确认 Sheet 流程，FileContentRenderer 在 file 类型消息点击时调用。
  /// null 表示不支持文件下载（测试场景等），FileCard 点击降级为无操作。
  final void Function(String fileId, String filename, String mimeType, int fileSize)? onFileTap;

  /// 文件下载状态快照表：fileId → (state, progress)。
  /// 由 ChatPage 注入。FileContentRenderer 构造 FileCard 时查此 map 决定
  /// downloadState/downloadProgress。null 或 map 中无对应 fileId 表示 notDownloaded。
  /// state 取值见 [FileDownloadSnapshot.state]。
  final Map<String, FileDownloadSnapshot>? fileDownloadSnapshots;

  /// 当前消息是否处于流式接收状态（LLM 仍在生成）。
  /// 默认 false，保证非流式场景向后兼容。renderer 可据此切换流式渲染分支
  /// （如逐字打字机效果 / 占位骨架）。
...
/// - 0: notDownloaded (未下载)
/// - 1: downloading (下载中，progress 必传)
/// - 2: downloaded (已下载)
/// - 3: uploading (上传中，仅发送方乐观消息用)
class FileDownloadSnapshot {
  final int state;
  final double progress; // 0.0 - 1.0，downloading/uploading 时用
  const FileDownloadSnapshot({required this.state, this.progress = 0});
}

/// 字符串首字母大写（用于卡片标题显示工具名:edit → Edit）。
/// 空串原样返回（避免 RangeError）。
