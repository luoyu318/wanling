...
class MessageBubble extends StatelessWidget {
  ...
  /// 透传给 FileContentRenderer 用于触发下载确认流程。
  final void Function(String fileId, String filename, String mimeType, int fileSize)? onFileTap;

  /// 文件下载状态快照表 (fileId → snapshot)。透传给 FileContentRenderer 决定
  /// FileCard 的 downloadState/downloadProgress。null 表示无下载态展示需求。
  ...
  /// 非多选模式的外层 padding(包 bubble 的 Row 用)。
  /// 默认 v8/h12(消息行内边距);MessageRow 接收方场景传 [EdgeInsets.zero]
  ...
  Widget build(BuildContext context) {
    ...
    // 状态指示器(sending/failed)+ 头像由 MessageRow 在外层组合,本 widget 不再处理。
...
class BubbleWithTail extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final color = isMe ? const Color(0xFF597BFF) : Colors.white;
    return Stack(
      ...
      children: [
        Container(
          ...
          constraints: BoxConstraints(
            // 0.95 屏宽兜底:多选模式(无外层 ConstrainedBox)和 dm_user_agent
            // (MessageRow 传 0.95)取此值;有头像场景由 MessageRow 传 0.7 显式限制。
