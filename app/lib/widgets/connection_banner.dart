...
///
/// **重连锁定（防重连闪烁）**：
/// 一旦 banner 已显示（断线超 3s），后续 backoff 重连发出的 `connecting` 不再
/// 隐藏 banner。原因：断线后 WS 状态循环 `disconnected → connecting(重连尝试)
/// → disconnected(握手失败)`，若 connecting 当成"恢复"立即隐藏，握手失败回
/// disconnected 又会显示，造成 banner 反复闪。锁定后只有真正 `connected` 才解锁。
///
/// 另有两层静默兜底（即便没到定时器也不显示）：
/// 1. 连接进行中（connecting）：仅首次连接(未显示过 banner)时视为非断线。
...
class _ConnectionBannerState extends ConsumerState<ConnectionBanner> {
  ...
  ///
  /// **重连锁定**:`_show=true` 后只有 connected 才解锁。重连尝试中的 connecting
  /// 仍视为断线(避免反复显示/消失闪烁)。
  bool _isDown(AsyncValue<ConnState> connState) {
    ...
    // banner 已显示后的锁定:connecting 视为仍在断线中。
    // 首次连接(_show=false)时 connecting 是正常握手预期,不算断线。
    if (s == ConnState.connecting) return _show;
  ...
  void _evaluate(AsyncValue<ConnState> connState) {
    ...
    if (down) {
      ...
      // 恢复(连接中[首次]/已连接/进入过渡期/loading):取消计时,立即隐藏。
