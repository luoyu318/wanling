...
import 'package:app/widgets/gallery/photo_view/photo_view.dart'
    show
        ...
        ScaleStateCycle;
import 'package:app/widgets/gallery/photo_view/src/controller/photo_view_controller.dart';
import 'package:app/widgets/gallery/photo_view/src/controller/photo_view_controller_delegate.dart';
import 'package:app/widgets/gallery/photo_view/src/controller/photo_view_scalestate_controller.dart';
import 'package:app/widgets/gallery/photo_view/src/core/photo_view_gesture_detector.dart';
import 'package:app/widgets/gallery/photo_view/src/core/photo_view_hit_corners.dart';
...
class PhotoViewCoreState extends State<PhotoViewCore>
  ...
  void onScaleEnd(ScaleEndDetails details) {
    ...
    // 松手 fling 惯性：按真实速度算位移（列表同款物理）。
    //
    // 原版 photo_view 写死 100px（direction * 100.0），无视速度，手感生硬。
    // 改为 velocity / drag：速度快滑得远，按摩擦力缓慢减速。
    // drag 越大阻尼越强、滑得越短。0.018：终位移 ≈ velocity × 55。
    if (_scaleBefore! / _scale == 1.0 && magnitude >= 400.0) {
      const double drag = 0.018;
      final Offset velocity = details.velocity.pixelsPerSecond;
      final Offset displacement = velocity / drag;
      animatePosition(
        _position,
        clampPosition(position: _position + displacement),
      );
    }
  }
  ...
  void animatePosition(Offset from, Offset to) {
    // 用 easeOut（慢→慢慢加快→慢慢减慢）替代 fling()（弹簧物理，
    // 曲线「快启动→快减速→弹性余震」，手感突兀，非列表式线性减速）。
    // easeOut 接近列表滑动的减速感。
