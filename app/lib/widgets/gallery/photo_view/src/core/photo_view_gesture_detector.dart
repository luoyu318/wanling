...
class PhotoViewGestureDetector extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // 双击缩放：用竞技场内 DoubleTapGestureRecognizer 与 TapGestureRecognizer
    // 仲裁。单击/双击落在同一 tap 手势上天然冲突，必须仲裁——单击需等
    // kDoubleTapTimeout≈300ms 确认「这不是双击的第一下」才会响应，双击则即时。
    // 这是行业标准（含主流 IM）的做法，靠动画掩盖延迟。原 pointer 层方案虽消除
    // 单击延迟，但第一次 tap 的 onTapUp 立即 close() 导致双击永远等不到第二下。
    if (onDoubleTap != null) {
