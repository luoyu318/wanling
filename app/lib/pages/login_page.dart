...
class _LoginPageState extends ConsumerState<LoginPage> {
  ...
  void initState() {
    super.initState();
    // 监听 savedLogins 变化,在 build 外同步 controller（避免 build 副作用）
    WidgetsBinding.instance.addPostFrameCallback((_) {
      _syncControllers(ref.read(savedLoginsProvider).selected);
    });
  }

  void _syncControllers(SavedLogin? selected) {
    if (selected != null) {
  ...
  Widget build(BuildContext context) {
    ...
    // controller 同步在 initState 中已处理，这里只读 selected 用于 UI 判断
