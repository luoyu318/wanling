...
class _FriendsTab extends ConsumerWidget {
  ...
  Future<void> _startDm(BuildContext context, WidgetRef ref, UserSummary u) async {
    try {
      final conv = await ref.read(apiProvider).createConversation(
            type: 'dm_user_user',
            memberUsernames: [u.username],
          );
      if (!context.mounted) return;
      context.pushReplacement(chatRoute(conv.id));
    } catch (e) {
      if (!context.mounted) return;
  ...
  void _confirmRemove(
    ...
    showAppDialog(
      ...
      confirmText: '删除',
      onConfirm: () {
        // removeFriend 乐观更新(先本地移除,再同步 server),失败时不回滚。
...
///
/// 每行:对方信息 + 「等待对方确认」副标题。
/// 申请一旦发出不可取消(只能等对方处理或超时),避免反复申请导致"已申请"误判。
