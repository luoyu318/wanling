...
void main() {
  ...
  testWidgets('点已选中卡片也能触发登录(SelectAccountPage 登出态场景)',
      (tester) async {
    // 复现 bug:登出后 selectedIndex 仍指向前次登录账号,
    // SelectAccountPage 点该已选中卡片应能用保存密码重新登录,
    // 不能像 switch_account_sheet 那样 no-op。
    SharedPreferences.setMockInitialValues({});
    final prefs = await SharedPreferences.getInstance();
    final storage = SecureStorage(deviceId: 'test-device');
    final loginCalls = <String>[];
    final selectedNotifier = SavedLoginsNotifier(
      prefs: prefs,
      storage: storage,
      onBaseUrlChange: (_) {},
      onLogout: ({bool silent = false}) async {},
    ...
    await selectedNotifier.add('http://x', 'u1', 'p1');
    selectedNotifier.select(0); // 模拟前次登录的选中态
    final c = ProviderContainer(
      overrides: [savedLoginsProvider.overrideWith((ref) => selectedNotifier)],
    );
    ...
    expect(loginCalls, ['u1:p1'],
        reason: 'SelectAccountPage 在登出态,点已选中卡片应用保存密码重新登录,'
            '不能像 switch_account_sheet 那样视作 no-op');
  });
