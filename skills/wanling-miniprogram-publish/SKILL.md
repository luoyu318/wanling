...
- **系统返回键**: 页内有历史 → 逐级回退;入口页 → **最小化到浮球**(不销毁,JS 继续跑)
- **胶囊 ⋯ 菜单**: 新增「浮窗」= 主动最小化;**⊙ 关闭** = 真正销毁实例
- **多任务**: 点浮球开卡片视图,可恢复/切换/上滑关闭;上限 5 个,超限 LRU 自动关最久未用
- **消息页下拉**: 宿主消息页下拉可拉出小程序面板(用户侧入口,与小程序无关)
- **状态存档(强烈建议)**: 游戏类小程序在关键节点写 localStorage 存档 + 启动恢复;
  `beforeunload` 在 WebView 销毁时不可靠,勿依赖;实例被 LRU 淘汰/⊙ 关闭后内存态丢失,
  localStorage(origin 级,按账号隔离)与 `wanling.storage` 云数据不受影响
