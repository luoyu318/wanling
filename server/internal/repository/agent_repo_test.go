...
func createTestAgent(t *testing.T, repo *AgentRepo, ownerID string) *model.Agent {
	t.Helper()
	a, err := repo.Create(t.Context(), ownerID, "testagent-"+shorten(t.Name()), "secret_xxx", "")
	if err != nil {
...
func TestAgentRepo_Update_BioPointer(t *testing.T) {
	...
	user, err := urepo.Create(t.Context(), uniqueShortName(t, "own_"), "$2a$10$hash")
	if err != nil {
	...
	emptyName := ""
	emptyAvatar := ""
	reloaded, err := repo.Update(t.Context(), agent.ID, emptyName, emptyAvatar, &newBio)
	if err != nil {
...
func TestAgentRepo_Update_BioNilKeepsValue(t *testing.T) {
	...
	repo := NewAgentRepo(db)
	urepo := NewUserRepo(db)
	user, _ := urepo.Create(t.Context(), uniqueShortName(t, "own_"), "$2a$10$hash")
	agent := createTestAgent(t, repo, user.ID)
	...
	reloaded, err := repo.Update(t.Context(), agent.ID, "新名字", "", nil)
	if err != nil {
		t.Fatalf("Update: %v", err)
	}
...
func TestAgentRepo_Update_BioEmptyStringClears(t *testing.T) {
	...
	emptyBio := ""
	reloaded, err := repo.Update(t.Context(), agent.ID, "", "", &emptyBio)
	if err != nil {
		t.Fatalf("Update: %v", err)
	}
...
func TestAgentRepo_ResetSecretKey_ChangesKey(t *testing.T) {
	...
	user, err := urepo.Create(t.Context(), uniqueShortName(t, "reset_"), "$2a$10$hash")
	if err != nil {
		t.Fatalf("Create user: %v", err)
	}
	origKey := randKey(t)
	agent, err := arepo.Create(t.Context(), user.ID, "ResetAgent", origKey, "")
	if err != nil {
		t.Fatalf("Create agent: %v", err)
	}

	newKey, err := arepo.ResetSecretKey(t.Context(), agent.ID)
	if err != nil {
	...
	after, err := arepo.GetByID(t.Context(), agent.ID)
	if err != nil {
...
func TestAgentRepo_ResetSecretKey_NonexistentReturnsError(t *testing.T) {
	...
	_, err := arepo.ResetSecretKey(t.Context(), "00000000-0000-0000-0000-000000000000")
	if err == nil {
		t.Fatal("期望返回错误（agent 不存在），实际 nil")
	}
}

// TestAgentRepo_CreateWithType 验证 Create 的 agentType 参数能落库,
// 且 GetByID 读回时 type 一致。覆盖 opencode 类型与默认空串两种。
func TestAgentRepo_CreateWithType(t *testing.T) {
	db := SetupTestDB(t)
	repo := NewAgentRepo(db)
	userRepo := NewUserRepo(db)

	user, err := userRepo.Create(t.Context(), uniqueShortName(t, "u"), "$2a$10$hash")
	if err != nil {
		t.Fatalf("Create user: %v", err)
	}

	t.Run("opencode type 落库", func(t *testing.T) {
		a, err := repo.Create(t.Context(), user.ID, uniqueShortName(t, "oc"), "sk", "opencode")
		if err != nil {
			t.Fatalf("Create agent: %v", err)
		}
		if a.Type != "opencode" {
			t.Errorf("Type = %q, want opencode", a.Type)
		}
	...
	t.Run("默认空串 type", func(t *testing.T) {
		a, err := repo.Create(t.Context(), user.ID, uniqueShortName(t, "norm"), "sk", "")
		if err != nil {
			t.Fatalf("Create agent: %v", err)
		}
		if a.Type != "" {
