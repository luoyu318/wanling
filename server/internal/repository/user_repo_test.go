...
func TestUserRepo_UpdatePassword_UpdatesHashAndAffectsLogin(t *testing.T) {
	...
	user, err := repo.Create(t.Context(), uniqueShortName(t, "upw_"), string(oldHash))
	if err != nil {
	...
	if err := repo.UpdatePassword(t.Context(), user.ID, string(newHash)); err != nil {
		t.Fatalf("UpdatePassword: %v", err)
	}

	// 重新查
	reloaded, err := repo.GetByID(t.Context(), user.ID)
...
func TestUserRepo_UpdatePassword_ReturnsErrorOnMissingUser(t *testing.T) {
	db := SetupTestDB(t)
	repo := NewUserRepo(db)
	err := repo.UpdatePassword(t.Context(), "00000000-0000-0000-0000-000000000000", "anyhash")
	if err == nil {
...
func TestUserRepo_Update_PartialFieldsOnlyUpdatesProvided(t *testing.T) {
	db := SetupTestDB(t)
	repo := NewUserRepo(db)
	user, err := repo.Create(t.Context(), uniqueShortName(t, "upd_"), "$2a$10$hash")
	if err != nil {
		t.Fatalf("Create: %v", err)
	}
	...
	newNick := "新昵称"
	reloaded, err := repo.Update(t.Context(), user.ID, UpdateUserParams{Nickname: &newNick})
	if err != nil {
...
func TestUserRepo_Update_EmptyStringClearsField(t *testing.T) {
	db := SetupTestDB(t)
	repo := NewUserRepo(db)
	user, err := repo.Create(t.Context(), uniqueShortName(t, "clr_"), "$2a$10$hash")
	if err != nil {
		t.Fatalf("Create: %v", err)
	}
	...
	// 传空串指针清空 bio
	emptyBio := ""
	reloaded, err := repo.Update(t.Context(), user.ID, UpdateUserParams{Bio: &emptyBio})
	if err != nil {
		t.Fatalf("Update: %v", err)
	}
...
func TestUserRepo_Update_AvatarEmptyStringIgnored(t *testing.T) {
	db := SetupTestDB(t)
	repo := NewUserRepo(db)
	user, err := repo.Create(t.Context(), uniqueShortName(t, "ava_"), "$2a$10$hash")
	if err != nil {
		t.Fatalf("Create: %v", err)
	}
	...
	reloaded, err := repo.Update(t.Context(), user.ID, UpdateUserParams{AvatarURL: ""})
	if err != nil {
		t.Fatalf("Update: %v", err)
	}
