...
func NewFileRepo(db *sql.DB) *FileRepo {
	return &FileRepo{queryExecutor: queryExecutor{db: db}}
}
...
func (r *FileRepo) Create(ctx context.Context, p CreateFileParams) (*File, error) {
	f := &File{}
	err := r.queryRow(ctx,
...
func (r *FileRepo) GetByID(ctx context.Context, id string) (*File, error) {
	f := &File{}
	err := r.queryRow(ctx,
...
func (r *FileRepo) AddConvLink(ctx context.Context, fileID, convID string) error {
	_, err := r.exec(ctx, `
...
// 放行条件任一为 true 即可下载(对齐主流 IM 授权模型,见 migration 018):
//   - IsOwner: claimer 是文件 owner(向后兼容老逻辑)
//   - IsAvatar: 文件是某 user/agent/group 的头像(社交公开属性,任何登录用户可看)
...
func (r FileAccessResult) IsAvatar() bool {
	return r.IsUserAvatar || r.IsAgentAvatar || r.IsConvAvatar
}
...
//
// 放行档(任一即 OK):
//  1. 头像白名单: file_id 出现在 users.avatar_url / agents.avatar_url /
//     conversations.avatar_url 中(社交公开属性,任何 participant 可看,
//     含群头像 — 群成员需正常看到群头像)
//  2. 会话 participant: claimer 是该 file 被引用(经 file_conv_links)的任一 conv 的 participant
//  3. owner 兜底: claimer 是该 file 的 owner_id
//
// 单次查询拿全判断,避免分多次 SQL 增加下载延迟。
func (r *FileRepo) CheckAccess(ctx context.Context, fileID, claimerID, memberType string) (FileAccessResult, error) {
	...
	err := r.queryRow(ctx, `
		SELECT
			...
			EXISTS(SELECT 1 FROM conversations WHERE avatar_url = '/api/files/' || f.id::text) AS is_conv_avatar,
			EXISTS(
	...
	).Scan(&res.IsOwner, &res.IsUserAvatar, &res.IsAgentAvatar, &res.IsConvAvatar, &res.IsConvParticipant)
	if errors.Is(err, sql.ErrNoRows) {
