...
// participantTestSeed 在已跑过 init 的 DB 上 seed 一组测试数据。
// 返回的 ids 供后续测试调用 ParticipantRepo 用。
//
// 注意:测试库 SetupTestDB 跑 001_init.sql(已合并 participants 模型),
// 直接是 participants 模型 schema,无需手动 migration。
...
// TestParticipantRepo_AddAndList 验证 AddParticipantsTx + ListByConversation + Exists:
...
func TestParticipantRepo_AddAndList(t *testing.T) {
	...
	got, err := repo.ListByConversation(t.Context(), seed.convID)
	if err != nil {
	...
	// 3. Exists 校验
	exists, err := repo.Exists(t.Context(), seed.convID, seed.userAID, "user")
	if err != nil {
	...
	exists2, err := repo.Exists(t.Context(), seed.convID, fakeUserID, "user")
	if err != nil {
		t.Fatalf("Exists 失败: %v", err)
	}
	if exists2 {
	...
	// 5. Get 校验完整字段
	p, err := repo.Get(t.Context(), seed.convID, seed.userAID, "user")
	if err != nil {
	...
	// 6. Get 不存在返 nil 不报错
	notExist, err := repo.Get(t.Context(), seed.convID, fakeUserID, "user")
	if err != nil {
...
func TestParticipantRepo_IncrUnreadExceptSender(t *testing.T) {
	...
	tx = beginTx(t, db)
	if err := repo.IncrUnreadTx(t.Context(), tx, seed.convID, seed.userAID, "user"); err != nil {
...
func TestParticipantRepo_MarkMessagesRead(t *testing.T) {
	...
	if err := repo.AddParticipantsTx(t.Context(), tx, seed.conv2ID, []ParticipantInput{
		{MemberID: seed.userAID, MemberType: "user", Role: "owner"},
	}); err != nil {
	...
	tx = beginTx(t, db)
	if err := repo.IncrUnreadTx(t.Context(), tx, seed.conv2ID, seed.agentID, "agent"); err != nil {
	...
	tx = beginTx(t, db)
	newUnread, err := repo.MarkMessagesReadTx(t.Context(), tx, seed.convID, seed.userAID, "user", msgIDs[:2])
	if err != nil {
	...
	tx = beginTx(t, db)
	newUnread2, err := repo.MarkMessagesReadTx(t.Context(), tx, seed.convID, seed.userAID, "user", msgIDs[2:])
	if err != nil {
...
func TestParticipantRepo_MarkMessagesRead_EmptyList(t *testing.T) {
	...
	tx = beginTx(t, db)
	newUnread, err := repo.MarkMessagesReadTx(t.Context(), tx, seed.convID, seed.userAID, "user", nil)
	if err != nil {
...
func TestParticipantRepo_SetPinned_SetHidden(t *testing.T) {
	...
	// 初始均为 NULL
	p, _ := repo.Get(t.Context(), seed.convID, seed.userAID, "user")
	if p.HiddenAt != nil || p.PinnedAt != nil {
		t.Fatalf("初始 hidden/pinned 应为 nil, 实际 hidden=%v pinned=%v", p.HiddenAt, p.PinnedAt)
	}

	// user_a 置顶
	if err := repo.SetPinned(t.Context(), seed.convID, seed.userAID, "user", true); err != nil {
	...
	pB, _ := repo.Get(t.Context(), seed.convID, seed.userBID, "user")
	if pB.PinnedAt != nil {
		t.Errorf("user_b pinned_at 不应被 user_a 的置顶影响, 实际 %v", pB.PinnedAt)
	}

	// user_b 隐藏
	if err := repo.SetHidden(t.Context(), seed.convID, seed.userBID, "user", true); err != nil {
	...
	// 取消置顶
	if err := repo.SetPinned(t.Context(), seed.convID, seed.userAID, "user", false); err != nil {
	...
	// 取消隐藏
	if err := repo.SetHidden(t.Context(), seed.convID, seed.userBID, "user", false); err != nil {
...
func TestParticipantRepo_RemoveParticipant(t *testing.T) {
	...
	// 踢 user_b(member 退群)
	tx = beginTx(t, db)
	if err := repo.RemoveParticipantTx(t.Context(), tx, seed.convID, seed.userBID, "user"); err != nil {
	...
	// user_b 应不在
	exists, _ := repo.Exists(t.Context(), seed.convID, seed.userBID, "user")
	if exists {
		t.Errorf("RemoveParticipant 后 user_b 仍存在")
	}
	// user_a 和 agent 应仍在
	existsA, _ := repo.Exists(t.Context(), seed.convID, seed.userAID, "user")
	if !existsA {
		t.Errorf("user_a 应仍在 conv1")
	}
	existsAg, _ := repo.Exists(t.Context(), seed.convID, seed.agentID, "agent")
	if !existsAg {
	...
	list, err := repo.ListByConversation(t.Context(), seed.convID)
	if err != nil {
		t.Fatalf("ListByConversation 失败: %v", err)
	}
	if len(list) != 2 {
...
func TestParticipantRepo_OwnerLeaveDestroyConversation(t *testing.T) {
	...
	tx = beginTx(t, db)
	if err := repo.DestroyConversationTx(t.Context(), tx, seed.convID); err != nil {
