...
func NewFriendshipRepo(db *sql.DB) *FriendshipRepo {
	return &FriendshipRepo{queryExecutor: queryExecutor{db: db}}
}
...
func (r *FriendshipRepo) CreateRequest(ctx context.Context, fromUserID, toUserID string) (*model.Friendship, error) {
...
func (r *FriendshipRepo) Accept(ctx context.Context, requestID, byUserID string) error {
	return r.advanceState(ctx, requestID, byUserID, "accepted", "friend_id")
}

// Reject 拒绝好友请求。只有 friend_id(接收方)才能拒绝。
func (r *FriendshipRepo) Reject(ctx context.Context, requestID, byUserID string) error {
	return r.advanceState(ctx, requestID, byUserID, "rejected", "friend_id")
}

// Cancel 取消好友请求(发起方主动撤回)。只有 user_id(发起方)才能取消。
func (r *FriendshipRepo) Cancel(ctx context.Context, requestID, byUserID string) error {
	return r.advanceState(ctx, requestID, byUserID, "canceled", "user_id")
}
...
func (r *FriendshipRepo) advanceState(ctx context.Context, requestID, byUserID, newState, roleCol string) error {
	res, err := r.exec(ctx, `
		UPDATE friendships
...
func (r *FriendshipRepo) AreFriends(ctx context.Context, userA, userB string) (bool, error) {
	var exists bool
	err := r.queryRow(ctx, `
		SELECT EXISTS(
...
func (r *FriendshipRepo) ListFriends(ctx context.Context, userID string) ([]string, error) {
...
func (r *FriendshipRepo) ListIncomingRequests(ctx context.Context, userID string) ([]model.Friendship, error) {
	return r.listRequests(ctx, userID, "friend_id")
}

// ListOutgoingRequests 返回该 user 发出的 pending 请求(user_id = user AND status = pending)。
func (r *FriendshipRepo) ListOutgoingRequests(ctx context.Context, userID string) ([]model.Friendship, error) {
	return r.listRequests(ctx, userID, "user_id")
}

// listRequests 是 ListIncoming/OutgoingRequests 的共享实现。
// roleCol: "friend_id"(incoming)或 "user_id"(outgoing)。
func (r *FriendshipRepo) listRequests(ctx context.Context, userID, roleCol string) ([]model.Friendship, error) {
	rows, err := r.query(ctx, `
...
func (r *FriendshipRepo) GetByID(ctx context.Context, requestID string) (*model.Friendship, error) {
	f := &model.Friendship{}
	err := r.queryRow(ctx, `
		SELECT id, user_id, friend_id, status, created_at, responded_at
