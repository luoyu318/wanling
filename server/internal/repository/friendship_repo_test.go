...
func TestFriendshipRepo_CreateRequest(t *testing.T) {
	...
	f, err := repo.CreateRequest(t.Context(), seed.userAID, seed.userBID)
	if err != nil {
...
func TestFriendshipRepo_CreateRequest_Duplicate(t *testing.T) {
	...
	if _, err := repo.CreateRequest(t.Context(), seed.userAID, seed.userBID); err != nil {
		t.Fatalf("首次 CreateRequest(a,b) 失败: %v", err)
	}

	// 反向重复(B→A)
	_, err := repo.CreateRequest(t.Context(), seed.userBID, seed.userAID)
	if !errors.Is(err, ErrFriendshipAlreadyExists) {
		t.Errorf("反向 CreateRequest(b,a) 期望 ErrFriendshipAlreadyExists, 实际 %v", err)
	}

	// 正向重复(A→B 再来一次)
	_, err = repo.CreateRequest(t.Context(), seed.userAID, seed.userBID)
	if !errors.Is(err, ErrFriendshipAlreadyExists) {
...
func TestFriendshipRepo_Accept(t *testing.T) {
	...
	if err := repo.Accept(t.Context(), f.ID, seed.userAID); !errors.Is(err, sql.ErrNoRows) {
		t.Errorf("user_a 自己接受应失败: 期望 sql.ErrNoRows, 实际 %v", err)
	}
	if err := repo.Accept(t.Context(), f.ID, seed.userCID); !errors.Is(err, sql.ErrNoRows) {
		t.Errorf("第三方 user_c 接受应失败: 期望 sql.ErrNoRows, 实际 %v", err)
	}

	// friend_id(user_b)调 Accept → 成功
	if err := repo.Accept(t.Context(), f.ID, seed.userBID); err != nil {
	...
	if err := repo.Accept(t.Context(), f.ID, seed.userBID); !errors.Is(err, sql.ErrNoRows) {
...
func TestFriendshipRepo_Reject_Cancel(t *testing.T) {
	...
	t.Run("Reject", func(t *testing.T) {
		...
		if err := repo.Reject(t.Context(), f.ID, seed.userAID); !errors.Is(err, sql.ErrNoRows) {
			t.Errorf("user_a 自己拒绝应失败: 期望 sql.ErrNoRows, 实际 %v", err)
		}

		// friend_id 调 Reject → 成功
		if err := repo.Reject(t.Context(), f.ID, seed.userBID); err != nil {
	...
	t.Run("Cancel", func(t *testing.T) {
		...
		if err := repo.Cancel(t.Context(), f.ID, seed.userBID); !errors.Is(err, sql.ErrNoRows) {
			t.Errorf("user_b(接收方)取消应失败: 期望 sql.ErrNoRows, 实际 %v", err)
		}

		// user_id 调 Cancel → 成功
		if err := repo.Cancel(t.Context(), f.ID, seed.userAID); err != nil {
...
func TestFriendshipRepo_AreFriends(t *testing.T) {
	...
	// 无任何关系 → false
	got, err := repo.AreFriends(t.Context(), seed.userAID, seed.userBID)
	if err != nil {
	...
	// pending → false
	f, _ := repo.CreateRequest(t.Context(), seed.userAID, seed.userBID)
	got, _ = repo.AreFriends(t.Context(), seed.userAID, seed.userBID)
	if got {
		t.Error("pending 时 AreFriends 应为 false")
	}
	// 反向也 false
	got, _ = repo.AreFriends(t.Context(), seed.userBID, seed.userAID)
	if got {
	...
	f2, _ := repo2.CreateRequest(t.Context(), seed2.userAID, seed2.userBID)
	if err := repo2.Reject(t.Context(), f2.ID, seed2.userBID); err != nil {
		t.Fatalf("Reject 失败: %v", err)
	}
	got, _ = repo2.AreFriends(t.Context(), seed2.userAID, seed2.userBID)
	if got {
	...
	f3, _ := repo3.CreateRequest(t.Context(), seed3.userAID, seed3.userBID)
	if err := repo3.Cancel(t.Context(), f3.ID, seed3.userAID); err != nil {
		t.Fatalf("Cancel 失败: %v", err)
	}
	got, _ = repo3.AreFriends(t.Context(), seed3.userAID, seed3.userBID)
	if got {
...
func TestFriendshipRepo_ListFriends(t *testing.T) {
	...
	fAB, _ := repo.CreateRequest(t.Context(), seed.userAID, seed.userBID)
	_ = repo.Accept(t.Context(), fAB.ID, seed.userBID)

	fAC, _ := repo.CreateRequest(t.Context(), seed.userAID, seed.userCID)
	_ = repo.Accept(t.Context(), fAC.ID, seed.userCID)

	_, _ = repo.CreateRequest(t.Context(), seed.userBID, seed.userCID) // pending,不计

	// user_a 的好友列表:应包含 b 和 c(顺序不保证,用集合校验)
	ids, err := repo.ListFriends(t.Context(), seed.userAID)
	if err != nil {
...
func TestFriendshipRepo_ListIncomingRequests(t *testing.T) {
	...
	fAB, _ := repo.CreateRequest(t.Context(), seed.userAID, seed.userBID)
	_, _ = repo.CreateRequest(t.Context(), seed.userCID, seed.userBID)
	...
	got, err := repo.ListIncomingRequests(t.Context(), seed.userBID)
	if err != nil {
...
func TestFriendshipRepo_ListOutgoingRequests(t *testing.T) {
	...
	got, err := repo.ListOutgoingRequests(t.Context(), seed.userAID)
	if err != nil {
...
func TestFriendshipRepo_RemoveFriend(t *testing.T) {
	...
	if err := repo.RemoveFriend(t.Context(), seed.userAID, seed.userBID); !errors.Is(err, sql.ErrNoRows) {
	...
	if err := repo.RemoveFriend(t.Context(), seed.userBID, seed.userAID); err != nil {
