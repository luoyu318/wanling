...
func seedGroup(t *testing.T) *seedGroupFixture {
	...
	creator, err := urepo.Create(t.Context(), shortName(t, "gcr"), "$2a$10$hash")
	if err != nil {
		t.Fatalf("Create creator: %v", err)
	}
	m1, err := urepo.Create(t.Context(), shortName(t, "gm1"), "$2a$10$hash")
	if err != nil {
		t.Fatalf("Create m1: %v", err)
	}
	m2, err := urepo.Create(t.Context(), shortName(t, "gm2"), "$2a$10$hash")
	if err != nil {
	...
	conv, err := crepo.CreateTx(t.Context(), tx, "group_user", "测试群", "")
	if err != nil {
		t.Fatalf("CreateTx: %v", err)
	}
	if err := prepo.AddParticipantsTx(t.Context(), tx, conv.ID, []repository.ParticipantInput{
...
func TestGroupHandler_InviteMember_HappyPath(t *testing.T) {
	f := seedGroup(t)
	// 另一个 user 作为被邀请人
	newUser, err := repository.NewUserRepo(f.db).Create(t.Context(), shortName(t, "invitee"), "$2a$10$hash")
	if err != nil {
...
func TestGroupHandler_InviteMember_NonParticipant_403(t *testing.T) {
	f := seedGroup(t)
	// 一个完全在群外的 user
	outsider, err := repository.NewUserRepo(f.db).Create(t.Context(), shortName(t, "outsider"), "$2a$10$hash")
	if err != nil {
		t.Fatalf("Create outsider: %v", err)
	}
	invitee, err := repository.NewUserRepo(f.db).Create(t.Context(), shortName(t, "invitee2"), "$2a$10$hash")
	if err != nil {
		t.Fatalf("Create invitee: %v", err)
	}
...
func TestGroupHandler_KickMember_TargetNotInConv(t *testing.T) {
	f := seedGroup(t)
	outsider, _ := repository.NewUserRepo(f.db).Create(t.Context(), shortName(t, "out"), "$2a$10$hash")
