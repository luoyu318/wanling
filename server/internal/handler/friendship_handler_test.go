...
func seedFriendship(t *testing.T) *seedFriendshipFixture {
	...
	from, err := urepo.Create(t.Context(), shortName(t, "ffrom"), "$2a$10$hash")
	if err != nil {
		t.Fatalf("Create from: %v", err)
	}
	to, err := urepo.Create(t.Context(), shortName(t, "fto"), "$2a$10$hash")
	if err != nil {
...
func TestFriendshipHandler_CreateRequest_HappyPath(t *testing.T) {
	...
	// to_user 摘要不含 id(防泄漏)
	toUser, ok := data["to_user"].(map[string]interface{})
	if !ok {
...
func TestFriendshipHandler_CreateRequest_DuplicateConflict(t *testing.T) {
	...
	post := func(callerID, toUsername string) (int, string) {
	...
	// 首次 A→B 应 200
	if code, body := post(f.from.ID, f.to.Username); code != http.StatusOK {
		t.Fatalf("首次请求状态码: %d, want 200, body: %s", code, body)
	}
	// 同方向重复 → 409
	if code, _ := post(f.from.ID, f.to.Username); code != http.StatusConflict {
		t.Errorf("同方向重复状态码: %d, want 409", code)
	}
	// 反向(B→A)也应 409
	if code, _ := post(f.to.ID, f.from.Username); code != http.StatusConflict {
...
func TestFriendshipHandler_Accept_HappyPath(t *testing.T) {
	f := seedFriendship(t)
	// 先建一个 pending 请求
	fr, err := f.friendRepo.CreateRequest(t.Context(), f.from.ID, f.to.ID)
	if err != nil {
...
func TestFriendshipHandler_Accept_NotReceiver_404(t *testing.T) {
	...
	// 第三个 user 试图接受
	other, err := f.userRepo.Create(t.Context(), shortName(t, "other"), "$2a$10$hash")
	if err != nil {
...
func TestFriendshipHandler_RemoveFriend_HappyPath(t *testing.T) {
	...
	if err := f.friendRepo.Accept(t.Context(), fr.ID, f.to.ID); err != nil {
...
func TestFriendshipHandler_ListIncoming(t *testing.T) {
	f := seedFriendship(t)
	if _, err := f.friendRepo.CreateRequest(t.Context(), f.from.ID, f.to.ID); err != nil {
		t.Fatalf("CreateRequest: %v", err)
	}
	...
	requests := AssertOkList(t, w, http.StatusOK)
	if len(requests) != 1 {
		t.Fatalf("incoming 数量 = %d, want 1", len(requests))
	}
...
func TestFriendshipHandler_ListOutgoing(t *testing.T) {
	...
	requests := AssertOkList(t, w, http.StatusOK)
	if len(requests) != 1 {
		t.Fatalf("outgoing 数量 = %d, want 1", len(requests))
	}
	entry := requests[0].(map[string]interface{})
...
func TestFriendshipHandler_ListFriends(t *testing.T) {
	...
	friends := AssertOkList(t, w, http.StatusOK)
	if len(friends) != 1 {
		t.Fatalf("好友数量 = %d, want 1", len(friends))
	}
	entry := friends[0].(map[string]interface{})
	if entry["username"] != f.to.Username {
		t.Errorf("好友 username = %v, want %s", entry["username"], f.to.Username)
	}
}
