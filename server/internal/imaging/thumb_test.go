...
// TestGenerateThumbnail_OversizedInputFailSoft 验证超限输入走 fail-soft（返 error，不崩）。
//
// GenerateThumbnail 用 LimitReader 截断超大输入，截断后的字节无法成功解码，
// 返回 error 供调用方跳过缩略图（不阻断上传流程）。
func TestGenerateThumbnail_OversizedInputFailSoft(t *testing.T) {
	// 构造一个超过 MaxDecodeBytes 的 reader：用一个永远不会 EOF 的 reader
	// （无限零字节流），LimitReader 会读满上限后停止，解码必然失败。
	infinite := endlessZeroReader{}
	_, _, _, err := GenerateThumbnail(infinite)
	if err == nil {
		t.Fatal("oversized input: want decode error, got nil")
	}
}

// endlessZeroReader 永远返回 0 字节，模拟无限大输入。
