...
// MaxDecodeBytes 缩略图解码读取上限。
//
// 防御纵深：即使调用方未限制 reader，超此上限的输入会被截断导致解码失败，
// 走 fail-soft 跳过缩略图（不阻断上传）。取 64MB 覆盖高清原图，远超恶意超大输入。
const MaxDecodeBytes int64 = 64 << 20
...
func GenerateThumbnail(reader io.Reader) (thumb []byte, srcW, srcH int, err error) {
	...
	//    限长读取防恶意超大输入耗尽内存：超 MaxDecodeBytes 后 decode 自然失败走 fail-soft。
	srcBytes, err := io.ReadAll(io.LimitReader(reader, MaxDecodeBytes))
	if err != nil {
