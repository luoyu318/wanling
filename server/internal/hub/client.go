...
type Client struct {
	...
	Conn          *websocket.Conn
	Send          chan []byte
	ctx           context.Context // per-conn ctx,WS 连接生命周期内有效;client 断开 → cancel
	mu            sync.Mutex
	seq           int64
...
// NewClient 创建新的客户端实例。ctx 为 per-conn ctx(由 ws_handler 从 r.Context 派生),
// client 生命周期内所有 repo 调用都用此 ctx,client 断开 → ctx cancel → 进行中的查询中止。
func NewClient(ctx context.Context, id, role string, conn *websocket.Conn) *Client {
	return &Client{
		ID:            id,
		Role:          role,
		Conn:          conn,
...
// Ctx 返回 per-conn ctx。hub / processor 等下游组件读 client.Ctx() 拿 ctx 做 repo 调用。
