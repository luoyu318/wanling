...
func seedHubParticipantDB(t *testing.T) (*Hub, string, string, string) {
	...
	if err := db.QueryRow(`
		...
		VALUES ($1, 'HubAgent', '', 'sk', $2) RETURNING id
...
// TestBroadcastConversationUpdateToUsers 验证 CONVERSATION_UPDATE(仅 user 版)
// 只发给 user 端,不发 agent 端——这是 OC→万灵 单向同步的物理断回环保障:
// 插件(agent 连接)触发 UpdateTitleAsAgent 后收不到自己刚写的广播,链路终止。
func TestBroadcastConversationUpdateToUsers(t *testing.T) {
	h, convID, userID, agentID := seedHubParticipantDB(t)
	...
	h.BroadcastConversationUpdateToUsers(convID, "新名", "")

	got := recvOne(t, user)
	if got.T != model.EventConversationUpdate {
		t.Fatalf("expected CONVERSATION_UPDATE, got %s", got.T)
	}
	var payload map[string]any
	if err := json.Unmarshal(got.D, &payload); err != nil {
		t.Fatalf("unmarshal payload: %v", err)
	...
	if payload["title"] != "新名" {
		t.Fatalf("expected title=新名, got %v", payload["title"])
	}
	if payload["conv_id"] != convID {
		t.Fatalf("expected conv_id=%s, got %v", convID, payload["conv_id"])
	}

	// agent 必须收不到,否则会形成回声循环
	recvNone(t, agent, "agent")
}

// TestBroadcastSessionMetaUpdateToUsers 验证 SESSION_META_UPDATE(仅 user 版):
// plugin 写完 session_meta 后,server 广播完整 meta 给 user 端实时刷新
// SessionMetaStrip / EnvMetaStrip;agent 端收不到(断回环 plugin→OC)。
func TestBroadcastSessionMetaUpdateToUsers(t *testing.T) {
	h, convID, userID, agentID := seedHubParticipantDB(t)
	...
	metaPayload := json.RawMessage(`{"mode":"build","model_id":"glm-5.2","cwd":"/home/u/proj","git_branch":"main"}`)
	h.BroadcastSessionMetaUpdateToUsers(convID, metaPayload)

	got := recvOne(t, user)
	if got.T != model.EventSessionMetaUpdate {
		t.Fatalf("expected SESSION_META_UPDATE, got %s", got.T)
	}
	if got.S == 0 {
		t.Fatal("seq should be allocated (>0)")
	}
	...
	metaField, ok := payload["session_meta"]
	if !ok {
		t.Fatalf("payload 缺 session_meta 字段")
	}
	metaObj, ok := metaField.(map[string]any)
	if !ok {
		t.Fatalf("session_meta 应为 map,实际 %T", metaField)
	}
	if metaObj["git_branch"] != "main" {
		t.Errorf("git_branch = %v, want main", metaObj["git_branch"])
	}
	if metaObj["cwd"] != "/home/u/proj" {
		t.Errorf("cwd = %v, want /home/u/proj", metaObj["cwd"])
	}
