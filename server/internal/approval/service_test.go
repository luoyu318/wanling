...
func (m *mockHub) SendApprovalDecided(initiatorID string, payload map[string]any) {
	m.decided = append(m.decided, payload)
}
func (m *mockHub) SendApprovalExpired(initiatorID string, payload map[string]any) {
...
func TestServiceDecideHappyPath(t *testing.T) {
	...
	user, _ := userRepo.Create(t.Context(), shortName(t, "u"), "$2a$10$hash")
	agent, _ := agentRepo.Create(t.Context(), user.ID, shortName(t, "a"), "secret", "")
	conv, _ := convRepo.FindOrCreateDM(t.Context(), "dm_user_agent", repository.DMMembers{
	...
	msg, err := msgRepo.Create(t.Context(), conv.ID, "agent", agent.ID, contentBytes)
	if err != nil {
	...
	a, err := approvalRepo.Create(t.Context(), model.Approval{
		...
		InitiatorType: "agent", InitiatorID: agent.ID,
	...
	// 校验 DB 也已落盘
	dbApproval, _ := approvalRepo.GetByID(t.Context(), a.ID)
...
func TestServiceDecideInvalidActionFails(t *testing.T) {
	...
	contentBytes, _ := json.Marshal(wrapper)
	msg, _ := msgRepo.Create(t.Context(), conv.ID, "agent", agent.ID, contentBytes)

	a, _ := approvalRepo.Create(t.Context(), model.Approval{
		MessageID: msg.ID, ConversationID: conv.ID,
		InitiatorType: "agent", InitiatorID: agent.ID,
		CardType: model.CardTypeCommand, Actions: cardData.Actions,
		ExpiresAt:  time.Now().Add(5 * time.Minute).UTC(),
