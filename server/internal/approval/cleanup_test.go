...
func (m *mockExpiredFinder) FindExpired(ctx context.Context, now time.Time) ([]*ExpiredApproval, error) {
...
func (m *mockMarker) MarkExpired(ctx context.Context, id string) error {
...
func TestCleanupOnceMarksExpired(t *testing.T) {
	finder := &mockExpiredFinder{items: []*ExpiredApproval{
		{ID: "a-1", MessageID: "m-1", ConvID: "c-1", InitiatorID: "agent-1", SessionKey: "k-1"},
		{ID: "a-2", MessageID: "m-2", ConvID: "c-2", InitiatorID: "agent-2", SessionKey: "k-2"},
	}}
	marker := &mockMarker{}
	hub := &mockHub{}

	cleanupOnce(t.Context(), finder, marker, hub, time.Now())
...
func TestCleanupOnceSkipsMarkerError(t *testing.T) {
	finder := &mockExpiredFinder{items: []*ExpiredApproval{{ID: "a-1", InitiatorID: "agent-1"}}}
