...
func TestOnlineOfflineIsOnline_Basic(t *testing.T) {
	...
	if p.IsOnline(ctx, "agent", "a1") {
		t.Fatal("未 Online 前应判定离线")
	}

	if err := p.Online(ctx, "agent", "a1"); err != nil {
	...
	if err := p.Offline(ctx, "agent", "a1"); err != nil {
...
func TestOnline_SetsTTL(t *testing.T) {
	...
	_ = p.Online(context.Background(), "agent", "a1")
...
func TestRefreshTTL_RenewsExistingKey(t *testing.T) {
	...
	// 续期，TTL 应回到接近 60s
	if err := p.RefreshTTL(ctx, "agent", "a1"); err != nil {
...
// TestOnline_CancelledCtx_ReturnsError 验证传入已 cancel 的 ctx 时，
// Redis 操作返回 ctx 错误（而非忽略 ctx）。
func TestOnline_CancelledCtx_ReturnsError(t *testing.T) {
	p, _, _ := newTestPresence(t)

	ctx, cancel := context.WithCancel(context.Background())
	cancel() // 立即 cancel

	_ = p.Online(ctx, "agent", "a1") // miniredis 对 ctx cancel 行为不保证；本测验证签名层面 ctx 正确传入
	// 不管 miniredis 行为如何，ctx 正确传入是签名层面的保证
}

// TestIsOnline_CancelledCtx_ReturnsFalse 验证传入已 cancel 的 ctx 时，
// IsOnline 返回 false（error path）。
func TestIsOnline_CancelledCtx_ReturnsFalse(t *testing.T) {
	p, _, _ := newTestPresence(t)

	ctx, cancel := context.WithCancel(context.Background())
	cancel()

	// 已 cancel 的 ctx 查询，应返 false（Redis 操作会因 ctx 错误失败）
	if p.IsOnline(ctx, "agent", "a1") {
		t.Fatal("已 cancel 的 ctx 查询应返 false")
	}
}
