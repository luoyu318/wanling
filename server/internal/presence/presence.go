...
func (p *Presence) Online(ctx context.Context, role, id string) error {
	if p == nil || p.rdb == nil {
		return nil
	}
...
func (p *Presence) Offline(ctx context.Context, role, id string) error {
	if p == nil || p.rdb == nil {
		return nil
	}
...
func (p *Presence) IsOnline(ctx context.Context, role, id string) bool {
	if p == nil || p.rdb == nil {
		return false
	}
...
func (p *Presence) RefreshTTL(ctx context.Context, role, id string) error {
	if p == nil || p.rdb == nil {
		return nil
	}
	return p.rdb.Set(ctx, p.key(role, id), "1", ttl).Err()
...
// Close 释放底层 Redis 连接。main.go 优雅关闭时调用。
// nil 安全(降级模式下 rdb 为 nil)。
