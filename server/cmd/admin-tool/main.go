...
func addUserCmd(args []string) {
	...
	// 重名检查
	existing, err := repo.GetByUsername(context.Background(), *username)
	if err != nil {
		fatal("查询失败: %v", err)
	}
	if existing != nil {
	...
	user, err := repo.Create(context.Background(), *username, string(hash))
	if err != nil {
		fatal("创建失败: %v", err)
	}
...
func resetPasswordCmd(args []string) {
	...
	if err := repo.UpdatePassword(context.Background(), user.ID, string(hash)); err != nil {
		fatal("更新失败: %v", err)
	}
