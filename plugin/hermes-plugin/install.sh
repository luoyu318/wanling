...
EOF
    ...
    [[ -n "$HOME_CONV" ]] && block="$block"$'\n'"WANLING_HOME_CONV=$HOME_CONV"
...
# adapter.py 读 extra(server_url/agent_id/secret_key/home_conv)。
...
run_config_mode() {
    ...
    cur_home=$(read_env_value "$ENV_FILE" "WANLING_HOME_CONV")
    ...
    prompt HOME_USER "Home user ID（可选,install.sh 自动转 conv_id）" "$cur_home"
...
run_pair_mode() {
    ...
    if [[ "$DRY_RUN" == "true" ]]; then
        ...
        create_resp=$(curl -sf -X POST "$SERVER/api/pair/tickets" \
            -H 'Content-Type: application/json' \
            -d '{"type":"hermes"}') \
            || die "无法连接 $SERVER（检查 URL / server 是否运行）"
        ticket_id=$(echo "$create_resp" | python3 -c "import sys,json;d=json.load(sys.stdin);d=d.get('data') or d;print(d['ticket_id'])" 2>/dev/null) \
...
"
    ...
    while true; do
        ...
        status=$(echo "$resp" | python3 -c "import sys,json;d=json.load(sys.stdin);d=d.get('data') or d;print(d.get('status',''))" 2>/dev/null)

        if [[ "$status" == "completed" ]]; then
            agent_id=$(echo "$resp" | python3 -c "import sys,json;d=json.load(sys.stdin);d=d.get('data') or d;print(d.get('agent_id',''))" 2>/dev/null)
            secret_key=$(echo "$resp" | python3 -c "import sys,json;d=json.load(sys.stdin);d=d.get('data') or d;print(d.get('secret_key',''))" 2>/dev/null)
            # 自动取 owner_conv_id 作为 home_conv(cron 投递目标)。
            # server 在 agent 创建时已建好默认 conv,pairing 响应直接返,无需 client find_or_create。
            owner_conv_id=$(echo "$resp" | python3 -c "import sys,json;d=json.load(sys.stdin);d=d.get('data') or d;print(d.get('owner_conv_id',''))" 2>/dev/null)
            if [[ -n "$owner_conv_id" && -z "$HOME_CONV" ]]; then
                HOME_CONV="$owner_conv_id"
            fi
    ...
    [[ -z "$HOME_CONV" ]] && warn "未设 WANLING_HOME_CONV,cron 任务无法投递"
...
# ─── 工具：用 user_id 拿 conv_id(写 .env 前的转换) ─────────────────────────
# 用户输入 WANLING_USER_ID(语义友好),install.sh 内部调 find_or_create_conv
# 转换成 conv_id 写入 WANLING_HOME_CONV(adapter / hermes cron 框架读这个)。
fetch_conv_id() {
    local user_id="$1"
    [[ -z "$user_id" || -z "$AGENT_ID" || -z "$SECRET_KEY" ]] && return 0

    info "用 user_id 拉取 conv_id..."
    local token_resp token conv_resp conv_id
    token_resp=$(curl -sf -X POST "$SERVER/api/agents/$AGENT_ID/token" \
        -H "Content-Type: application/json" \
        -d "{\"secret_key\":\"$SECRET_KEY\"}" 2>/dev/null) || {
        warn "换 token 失败,跳过 conv_id 拉取(.env 中 WANLING_HOME_CONV 将留空)"
        return 0
    }
    token=$(echo "$token_resp" | python3 -c "import sys,json;d=json.load(sys.stdin);d=d.get('data') or d;print(d.get('token',''))" 2>/dev/null)
    [[ -z "$token" ]] && { warn "解析 token 失败"; return 0; }

    conv_resp=$(curl -sf -X POST "$SERVER/api/agents/me/conversations" \
        -H "Authorization: Bearer $token" \
        -H "Content-Type: application/json" \
        -d "{\"user_id\":\"$user_id\"}" 2>/dev/null) || {
        warn "find_or_create_conv 失败,跳过 conv_id 拉取"
        return 0
    }
    conv_id=$(echo "$conv_resp" | python3 -c "import sys,json;d=json.load(sys.stdin);d=d.get('data') or d;print(d.get('id',''))" 2>/dev/null)
    if [[ -n "$conv_id" ]]; then
        HOME_CONV="$conv_id"
        ok "✓ conv_id: $conv_id"
    else
        warn "解析 conv_id 失败(.env 中 WANLING_HOME_CONV 将留空)"
    fi
}
...
run_install_mode() {
    ...
    if [[ "$REGISTER_MODE" == "true" ]]; then
        ...
        if [[ "$DRY_RUN" != "true" ]]; then
            ...
            # 解析 JSON，失败打印原始响应
            if ! AGENT_ID=$(echo "$RESP" | python3 -c "import sys,json;d=json.load(sys.stdin);d=d.get('data') or d;print(d['id'])" 2>/dev/null) \
               || ! SECRET_KEY=$(echo "$RESP" | python3 -c "import sys,json;d=json.load(sys.stdin);d=d.get('data') or d;print(d['secret_key'])" 2>/dev/null); then
    ...
    [[ -z "$HOME_USER" ]] && warn "未设 Home user ID,cron 任务无法投递(按需手动设)"
