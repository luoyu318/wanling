...
login() { curl -sf -X POST "$BASE/api/auth/login" -H 'Content-Type: application/json' -d "{\"username\":\"$1\",\"password\":\"$2\"}" | python3 -c 'import sys,json;d=json.load(sys.stdin);print(d["data"]["token"])'; }
...
curl -sf "$BASE/api/mini-programs" -H "Authorization: Bearer $TOKEN_B" | grep -q "\"id\":\"$MP_ID\"" && { echo "   FAIL: B 看到了 private"; exit 1; } || echo "   OK"
...
curl -sf "$BASE/api/mini-programs/$MP_ID/package" -H "Authorization: Bearer $TOKEN_B" -o /tmp/mp_verify.zip
ACTUAL_SHA=$(sha256sum /tmp/mp_verify.zip | cut -d' ' -f1)
