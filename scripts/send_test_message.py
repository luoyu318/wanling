...
def fetch_token(server: str, agent_id: str, secret_key: str) -> str:
    ...
    try:
        with urllib.request.urlopen(req, timeout=10) as resp:
            ...
            # 兼容新 envelope {ok, data: {token}} 和老 envelope {token}
...
def find_or_create_conv(server: str, token: str, user_id: str) -> str:
    """agent JWT 调 POST /api/agents/me/conversations 拿 conv_id。

    用 user_id 反查/建 dm_user_agent 会话,返 conv_id(WS 协议层用)。
    """
    url = f"{server.rstrip('/')}/api/agents/me/conversations"
    ...
    try:
        with urllib.request.urlopen(req, timeout=10) as resp:
            data = json.loads(resp.read().decode())
            # 兼容老 envelope(裸对象)和新 envelope(data 字段)
            conv_data = data.get("data", data) if isinstance(data, dict) else {}
            conv_id = conv_data.get("id") if isinstance(conv_data, dict) else None
            if not conv_id:
                raise RuntimeError(f"find_or_create_conv 响应缺 id: {data}")
            return conv_id
    except HTTPError as e:
        body_text = ""
        try:
            ...
            pass
        raise RuntimeError(
            f"find_or_create_conv 失败: HTTP {e.code} {e.reason} {body_text}"
        )
    except URLError as e:
        raise RuntimeError(f"连接服务器失败: {e}")


async def send_message(ws, conv_id: str, text: str) -> None:
    """发一条 MESSAGE_CREATE(新协议:含 conversation_id)。"""
    payload = {
        "op": OP_DISPATCH,
...
async def run(args: argparse.Namespace) -> None:
    ...
    async with websockets.connect(ws_url) as ws:
        ...
        # 拿 conv_id(WS 协议层走 conversation_id)
        conv_id = find_or_create_conv(args.server, token, args.user_id)
        ...
        try:
            for i in range(1, args.count + 1):
                ...
                await send_message(ws, conv_id, text)
